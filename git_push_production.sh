...
# Web-Marbles Game - Railway Docker Deploy Script
...
echo -e "${GREEN}🚀 Web-Marbles - Начинаем автоматический коммит и пуш в production ветку${NC}"
...
if [ -n "$1" ]; then
    ...
    COMMIT_MESSAGE="Auto-deploy Web-Marbles: $TIMESTAMP"
fi
...
if git push origin production; then
    ...
    echo -e "${GREEN}🎮 Web-Marbles готов к развертыванию на Railway${NC}"
...
echo -e "${BLUE}  🔧 Dockerfile настроен для Node.js с Python и node-gyp${NC}"
echo -e "${BLUE}  🗄️  SQLite база данных с persistent volumes${NC}"
echo -e "${BLUE}  📦 railway.json настроен для Docker builder${NC}"
echo -e "${BLUE}  🎯 Marble Racing игра будет доступна по URL Railway сервиса${NC}"
echo -e "${BLUE}  🔌 WebSocket поддержка для реального времени${NC}"
echo -e "${BLUE}  👥 Поддержка до 250 игроков одновременно${NC}"
